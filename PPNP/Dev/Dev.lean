...
Defines the UD state space `OmegaUD` (occupancy vectors summing to M) and shows
it is equivalent to `SymFin N M` (multisets of size M over Fin N).
This section remains unchanged as it defines the system itself.
-/
